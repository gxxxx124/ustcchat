...
# 导入认证相关模块
from auth import create_users_table, set_global_pool, get_current_admin_user, get_current_user, UserResponse
...
class GPUResourceManager:
    ...
    def get_ollama_model(self):
        ...
        if "ollama" not in self.model_instances:
            ...
            logger.info(f"🚀 使用Open WebUI API模型: {model_name}")
...
# 添加上传页面路由（需要管理员权限）
@app.get("/upload.html")
async def upload_page(current_user: UserResponse = Depends(get_current_admin_user)):
